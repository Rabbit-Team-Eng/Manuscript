...
struct GetAllWorkspacesRequest {
    ...
    func getRequest(with pageNumber: Int, pageSize: Int) -> URLRequest {
        let urlString = "\(environment.rawValue)\(Endpoint.workspace)?PageSize=\(pageSize)&PageNumber=\(pageNumber)"
