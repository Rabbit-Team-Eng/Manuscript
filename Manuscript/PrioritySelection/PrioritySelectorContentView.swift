//
//  PrioritySelectorContentView.swift
//  Manuscript
//
...
class PrioritySelectorContentView: UIView, UIContentView {
    ...
    @objc private func priorityDidSelected(_ sender: UIView) {
        if let delegate = delegate, let model = model {
            delegate.actionDidHappen(action: .priorityShouldChange(currentPriority: model))
        }
    }
    
    private func applyConfiguration(configuration: UIContentConfiguration) {
        
        guard let config = configuration as? PrioritySelectorContentConfiguration, let model = config.model else { return }
        self.model = model
